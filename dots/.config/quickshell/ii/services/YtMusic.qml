...
Singleton {
    ...
    function _notifyMediaAction(action: string): void {
        // Our shell has no media OSD; wired as a no-op extension point.
    }
...
`]
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: line => {
                if (line.trim()) {
                    root.googleError = Translation.tr("YT Music runtime unavailable (yt-dlp, mpv, socat, deno o ytmusicapi faltan)")
        ...
        onExited: (code) => {
            ...
            if (root._pendingAvailabilityItem) {
                ...
                if (root.available) {
                    ...
                    root.loading = false
                    root.error = Translation.tr("YT Music runtime unavailable (yt-dlp, mpv, socat, deno o ytmusicapi faltan)")
                }
            }
        }
    }
    ...
    IpcHandler {
        ...
        function previous(): void {
            ...
            root.playPrevious()
            if (Config.options?.osd?.mediaEnabled ?? true)
                root._notifyMediaAction("previous")
        }
