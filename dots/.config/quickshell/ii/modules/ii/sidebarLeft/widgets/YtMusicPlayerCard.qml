...
Item {
    ...
    // Cava visualizer - same signal handling as the media controls panel
    Process {
        id: cavaProcess
        running: root.visible && (root.isPlaying || root.hasPlayer) && GlobalStates.sidebarLeftOpen && Appearance.effectsEnabled
        onRunningChanged: if (!cavaProcess.running) root.visualizerPoints = []
