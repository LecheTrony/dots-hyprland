...
Item {
    ...
    function commitPosition(idx, newX, newY) {
        const before = monitorConfig.monitors.map(m => ({ name: m.name, x: m.x, y: m.y }))
        const normalized = computeNormalized(monitorConfig.monitors, idx, newX, newY)
        ...
        // Persist every monitor whose position changed (normalization can shift them all).
        for (let i = 0; i < normalized.length; i++) {
            if (normalized[i].name === before[i]?.name && (normalized[i].x !== before[i].x || normalized[i].y !== before[i].y)) {
                monitorConfig.updateMonitor(i, { x: normalized[i].x, y: normalized[i].y })
                monitorConfig.save(i)
            }
        }
    }
